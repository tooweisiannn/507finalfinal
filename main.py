...
fig_violin.update_layout(xaxis_title="Stratification",
                         yaxis_title="Percentage of Severe Poverty")
...
fig_unemployment.update_layout(xaxis_title="Stratification",
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.P("We now consider some potential factors that may be driving the conclusions from the previous two graphs. This graph visualizes the average length bewteen two doctor visits. One interesting observation is that West Virginia and Tennesee both had a large number of days between checkups; this may have a direct correlation between the percentage of depression among adults within these states, however further investigation must be done in order to prove this correlation. Unexpectedly, New Mexico had the smallest number of days between checkups."),
        ...
        html.P("We now visualize the overall unemployment rates by stratification. Once again, we see that American Indian or Alaska Natives, Black, and Hawaiian or Pacific Islanders have the highest levels of unemployment. On the other hand, once again we see that Asians and White had the lowest rates of unemployments. This is similar, as we see thatWe cannot conclude any causation from one another, but it is interesting to note such statistic."),
    ...
    html.H2("Depression Rates Prediction with Ridge Regression"),
        html.P("In this section, we predict the percentage of depression given a stratification, location, and year. We utilize a ridge-regression to counter highly correlated features, along with a transformation of the dependent variable which transforms the range of the dependent variable to between 0 and 1, representing the percentage of adults that suffer from depression."),
        html.Div([
    ...
    html.H2("Depression Rates Prediction with Catboost"),
        html.P("In this section, we predict the percentage of depression given a stratification, location, and year. We utilize a catboost, a boosting library that supports categorical features. In particular, we implement a catboost regression. We choose to implement both a catboost and a ridge regression for the same task to compare the two machine learning models together. What we find is that catboost is much more conservative and more accuracte, with a MSE of around 4 while the Ridge Regression had a MSE around 13."),
        html.Div([
            html.Label("Select Year:"),
...
def update_plot(strat):
    # Use the GP function to generate the plot based on selected stratification and year
    if strat in gp_data.columns:
        filename = f'{strat}_gp_plot.pickle'
        # Load the figure from the pickle file
        fig = load_figure_from_pickle(filename)
